...
import {
  Plus, Edit3, Trash2, X, Check, Clock, AlertCircle,
} from 'lucide-react';
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
              <div className={styles.formGrid}>
                <div className={styles.field}>
                  ...
                  <input id="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={styles.input} placeholder="Ej: Corte caballero" />
                </div>

                <div className={styles.field}>
                  <label htmlFor="category">Categoria</label>
                  <select id="category" value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })} className={styles.select}>
                ...
                <div className={styles.field}>
                  ...
                  <input id="duration" type="number" min="1" value={form.duration_minutes} onChange={(e) => setForm({ ...form, duration_minutes: e.target.value })} className={styles.input} placeholder="Ej: 30" />
                </div>
                ...
                <div className={styles.fieldFull}>
                  ...
                  <textarea id="description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={styles.textarea} rows={2} placeholder="Descripcion del servicio..." />
                </div>
              </div>
                ...
                {services.length === 0 ? (
                  ...
                  return (
                    <tr key={service.id} className={!service.active ? styles.inactive : ''}>
                      ...
                      <td className={styles.priceCell}>{formatPrice(service.price_cents)}</td>
                      <td>
                      ...
                      <td className={styles.actionsCol}>
                        <div className={styles.rowActions}>
                          <button className={styles.iconBtn} onClick={() => handleEdit(service)} title="Editar">
                            <Edit3 size={14} />
                          </button>
                          {service.active && (
                            <button className={styles.iconBtnDanger} onClick={() => handleDelete(service.id)} title="Eliminar">
                              <Trash2 size={14} />
                            </button>
                          )}
                        </div>
